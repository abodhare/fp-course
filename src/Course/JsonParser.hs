...
jsonString ::
  Parser Chars
jsonString = between (is '"') (is '"') (list (specialParser ||| hexParser ||| nonSpecialCharacters))
  where specialParser = fromSpecialCharacter <$> specialCharacter
        hexParser = is '\\' >> hexu
        nonSpecialCharacters = satisfyAll (listh [isPrint, (/='"'), (/='\\')])
...
jsonObject ::
  Parser Assoc
jsonObject = betweenSepbyComma '{' '}' (lift2 (,) jsonString (spaces >> charTok ':' >> jsonValue))
...
readJsonValue x = readFile x >>= (return . parse jsonValue)
