...
instance Functor ListZipper where
  f <$> (ListZipper l x y) = ListZipper (f <$> l) (f x) (f <$> y)
...
findLeft f (ListZipper l x r) = case break f l of
    (_, Nil)          -> IsNotZ
    (l', (lx :. lxs)) -> IsZ (ListZipper lxs lx (reverse l' ++ (x:.Nil) ++ r))
...
findRight f (ListZipper l x r) = case break f r of
    (_, Nil)          -> IsNotZ
    (r', (rx :. rxs)) -> IsZ (ListZipper (reverse r' ++ (x:.Nil) ++ l) rx rxs)
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper (lx:.lxs) x r) = ListZipper lxs lx (x:.r)
moveLeftLoop (ListZipper Nil x r) = case reverse r of
    Nil         -> ListZipper Nil x Nil
    (rx :. rxs) -> ListZipper (rxs ++ (x :. Nil)) rx Nil
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l x (rx:.rxs)) = ListZipper (x:.l) rx rxs
moveRightLoop (ListZipper l x Nil) = case reverse l of
    Nil         -> ListZipper Nil x Nil
    (lx :. lxs) -> ListZipper Nil lx (lxs ++ (x:.Nil))
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft (ListZipper (lx:.lxs) x r) = IsZ (ListZipper lxs lx (x:.r))
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight (ListZipper l x (rx:.rxs)) = IsZ (ListZipper (x:.l) rx rxs)
...
swapLeft (ListZipper (lx:.lxs) x r) = IsZ (ListZipper (x:.lxs) lx r)
...
swapRight (ListZipper l x (rx:.rxs)) = IsZ (ListZipper l rx (x:.rxs))
...
moveLeftN n x
  ...
  | otherwise = g n x
    where g a (ListZipper l y r) = case drop (a - 1) l of
                                    Nil -> IsNotZ
                                    (z:.zs) -> IsZ (ListZipper zs z (reverse (take (a - 1) l) ++ (y:.Nil) ++ r))
...
moveRightN n x
  ...
  | otherwise = g n x
    where g a (ListZipper l y r) = case drop (a - 1) r of
                                    Nil -> IsNotZ
                                    (z:.zs) -> IsZ (ListZipper (reverse (take (a - 1) r) ++ (y:.Nil) ++ l) z zs)
...
moveLeftN' n x@(ListZipper l _ r) = case moveLeftN n x of
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n x@(ListZipper l _ r) = case moveRightN n x of
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n lst@(ListZipper l _ _)
  | n < 0 = IsNotZ
  | n < length l = moveLeftN (length l - n) lst
  | otherwise = moveRightN (n - length l) lst
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end lst@(ListZipper _ _ r) = case moveRightN (length r) lst of
  IsZ x -> x
  IsNotZ -> lst
...
start ::
  ListZipper a
  -> ListZipper a
start lst@(ListZipper l _ _) = case moveLeftN (length l) lst of
  IsZ x -> x
  IsNotZ -> lst
...
-- /Tip:/ Use `zipWith`
  (ListZipper fl fx fr) <*> (ListZipper l x r) = ListZipper (zipWith ($) fl l) (fx x) (zipWith ($) fr r)
...
instance Extend ListZipper where
  f <<= x = ListZipper (unfoldr (g . moveLeft) x) (f x) (unfoldr (g . moveRight) x)
    where g = (<$>) (\y -> (f y, y) ) . toOptional
